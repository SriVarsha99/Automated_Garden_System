...
public class Whiteflies extends Parasite {
    ...
    public Whiteflies(String name, int damage , String imageName, ArrayList<String> affectedPlants) {
    ...
    public void affectPlant(Plant plant) {
        if (random.nextDouble() >= MISS_CHANCE) {
            ...
            logger.info("Whiteflies have successfully damaged the plant {} at position ({}, {}). Old health: {}. New health: {}",
                    plant.getName(), plant.getRow(), plant.getCol(), oldHealth, newHealth);

        } else {
            // If missed, do nothing
            logger.info("Whiteflies attempted to damage the plant {} at position ({}, {}) but missed.",
