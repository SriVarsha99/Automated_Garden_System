...
public class CornEarworms extends Parasite {
    ...
    public CornEarworms(String name, int damage , String imageName, ArrayList<String> affectedPlants) {
    ...
    public void affectPlant(Plant plant) {
        if (random.nextDouble() >= MISS_CHANCE) {
            ...
            logger.info("Corn Earworms have successfully damaged the plant {} at position ({}, {}). Old health: {}. New health: {}",
                    plant.getName(), plant.getRow(), plant.getCol(), oldHealth, newHealth);

        } else {
            // If missed, do nothing
            logger.info("Corn Earworms attempted to damage the plant {} at position ({}, {}) but missed.",
